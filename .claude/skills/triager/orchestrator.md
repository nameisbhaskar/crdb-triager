...
Launch **two** agents simultaneously using the Task tool with **general-purpose** subagent type:
...
**Implementation:**
```
Send a SINGLE message with TWO Task tool calls to run agents in parallel.
This is critical for performance - both agents run concurrently.
...
2. Cross-validate evidence across both analyses
...
Use the Task tool to launch both agents concurrently:

**Important:** Send a SINGLE message with TWO Task tool uses for parallel execution.

```
I need to triage issue #156490. I'll launch two analysis agents in parallel:
...
Both agents will work independently and produce their outputs.
...
Once both analyses are complete:
...
**Total end-to-end:** ~8 minutes (parallel) vs ~13 minutes (sequential)
