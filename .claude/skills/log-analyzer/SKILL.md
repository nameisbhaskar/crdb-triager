...
### 5.1. 🚨 CRITICAL: Detect Test Timeouts BEFORE Analyzing Goroutine Leaks

**THIS PREVENTS MISCLASSIFYING TIMEOUTS AS SHUTDOWN BUGS**

**Phase 5.1a: Check for timeout indicators in test.log**

```bash
# Search for explicit timeout messages
Grep -i "test timed out\|timeout exceeded\|deadline exceeded\|panic: test timed out" test.log

# Search for timeout-related patterns
Grep -i "ran too long\|exceeded.*minute\|SIGQUIT.*timeout" test.log

# Check test duration if available
Grep -i "PASS\|FAIL.*[0-9]+\.[0-9]+s\|elapsed.*[0-9]+m" test.log
```

**Phase 5.1b: Calculate test duration**

```bash
# Extract start and end timestamps from test.log
# Compare total duration against known timeout limits:
# - Go tests: typically 10-15 minutes default
# - Roachtests: varies by test configuration
# - If duration ≈ timeout limit (within 10 seconds), likely timeout
```

**Common timeout limits:**
- Standard Go test: 10 minutes
- Go test with `-timeout` flag: check test configuration
- Roachtest: varies (check test definition)
- Import/workload tests: often 15 minutes

**Phase 5.1c: Analyze test activity leading to timeout**

```bash
# Look for slow operations in the last 5-10 minutes before failure
Grep -i "import\|loading\|fixture\|tpcc\|workload" test.log | tail -50

# Check for long-running operations
Grep -i "took [0-9]+m\|duration:.*m\|elapsed.*m" test.log

# Look for test progress indicators
# Example: "importing tpcc took 8m30s" → test spent most time on import
```

**Phase 5.1d: Distinguish timeout from goroutine leak**

**TIMEOUT SCENARIO (test killed mid-execution):**
```markdown
**Indicators:**
- Test duration ≈ timeout limit (e.g., ~15 minutes)
- Goroutine dump shows goroutines doing ACTIVE WORK (not just waiting)
- No "leaktest.AfterTest" or "cluster.Close()" in stack traces
- Many goroutines in "running" or "runnable" state
- Recent log activity shows test was making progress
- Slow operation completed just before timeout (e.g., "import took 8m30s")

**Example from issue #161919:**
- Test timeout: 15 minutes
- TPCC import took: 8 minutes 30 seconds
- Other operations took: ~6 minutes
- Total: ~14.5+ minutes → exceeded 15-minute timeout
- Goroutines dumped: Test was killed mid-execution, not after clean shutdown
- Classification: **TEST_TOO_SLOW** or **PERFORMANCE_REGRESSION**, NOT goroutine leak
```

**ACTUAL GOROUTINE LEAK (shutdown bug):**
```markdown
**Indicators:**
- Test attempted clean shutdown (cluster.Close(), s.Stopper().Stop())
- "leaktest.AfterTest" appears in stack traces
- Goroutines stuck WAITING (select, sync.Cond.Wait, chan receive)
- Long wait times (>1 minute) in idle states
- Shutdown completed but goroutines remain
- Test duration well under timeout limit

**Example:**
- Test duration: 2 minutes (well under 15-minute limit)
- Stack shows: "defer leaktest.AfterTest(t)()" was called
- Goroutines stuck at: sync.Cond.Wait for 5+ minutes
- Classification: **ACTUAL_BUG** (shutdown coordination failure)
```

**Phase 5.1e: Look for performance indicators**

```bash
# Search for import/workload performance metrics
Grep -i "import.*took\|fixture.*loaded\|workload.*duration" test.log

# Check for slow queries or operations
Grep -i "slow.*query\|slow.*request\|high latency" test.log

# Look for resource warnings before timeout
Grep -i "memory.*high\|cpu.*throttl\|disk.*slow" test.log
```

**Decision Tree:**

```
IF (test_duration ≈ timeout_limit AND slow_operation_found):
    classification_hint = TEST_TOO_SLOW or PERFORMANCE_REGRESSION
    reason = "Test timed out due to slow operation, not shutdown bug"

ELSE IF (shutdown_attempted AND goroutines_waiting AND duration << timeout):
    classification_hint = ACTUAL_BUG
    reason = "Goroutines leaked after clean shutdown attempt"

ELSE IF (no_shutdown_attempted AND goroutines_active):
    classification_hint = TEST_TIMEOUT
    reason = "Test killed mid-execution, goroutines were actively working"
```

**Critical Output for SYNTHESIS:**

At the end of LOG_ANALYSIS.md, include this section if timeout detected:
...
**Margin:** 15 seconds → **TIMEOUT LIKELY**

**Slow Operations Detected:**
1. TPCC import took 8m30s (test.log:1234)
2. Consistency checks took 3m15s (test.log:2345)
3. Other operations took ~3m00s
4. Total: ~14m45s → exceeds 15-minute timeout

**Goroutine Dump Context:**
- Dump taken at: Test termination (SIGQUIT due to timeout)
- Goroutines were: Actively working, not leaked after shutdown
- No shutdown attempted: No "leaktest.AfterTest" or "cluster.Close()" in stacks

**CRITICAL FOR SYNTHESIS:**
This is NOT a goroutine leak bug. This is a test timeout due to slow TPCC import.

**Classification Guidance:**
- PRIMARY: TEST_TOO_SLOW (test configuration issue)
- ALTERNATIVE: PERFORMANCE_REGRESSION (if import used to be faster)
- NOT: ACTUAL_BUG (goroutine leak / shutdown coordination)

**Recommended Action:**
1. Increase test timeout OR reduce workload fixture size
2. Investigate why TPCC import took 8m30s (expected: <5m?)
3. Consider splitting test into smaller tests
```

### 6. Analyze Goroutine Dumps (DEEP CONCURRENCY ANALYSIS)

**⚠️ IMPORTANT: Only analyze goroutines as "leaks" if timeout was NOT detected in section 5.1**
...
```bash
# ⚠️ CRITICAL: First confirm this is NOT a timeout (see section 5.1)
# If timeout detected, goroutines are NOT leaked - test was killed mid-execution

# If goroutine dump is after cluster.Close() AND no timeout:
...
# If timeout detected (test killed):
# - Goroutines are expected to be running (test was interrupted)
# - Focus on WHY test was slow, not why goroutines exist
# - Classification: TEST_TOO_SLOW, not GOROUTINE_LEAK
```
...
## 🚨 Timeout Analysis (If Applicable)

**⚠️ PERFORM THIS ANALYSIS FIRST - before analyzing goroutines as "leaks"**

**Timeout Detected:** [YES/NO]
**Test Duration:** [actual duration if calculable]
**Timeout Limit:** [known or estimated timeout limit]
**Margin:** [difference between duration and limit]
...
1. **TPCC import took 8m30s** (test.log:1234)
   - Expected: <5 minutes
   - Overhead: 3m30s over expected
2. **Consistency checks took 3m15s** (test.log:2345)
3. **Total estimated duration: 14m45s** → exceeds 15-minute timeout

### Goroutine Dump Context
- **Dump trigger:** Test timeout (SIGQUIT)
- **Goroutines state:** Actively working, not leaked after shutdown
- **Shutdown attempted:** NO - test was killed mid-execution
- **Stack traces show:** No "leaktest.AfterTest" or "cluster.Close()" calls

### Classification Guidance
- **PRIMARY:** TEST_TOO_SLOW (test needs longer timeout or smaller workload)
- **ALTERNATIVE:** PERFORMANCE_REGRESSION (if operation used to be faster)
- **NOT:** ACTUAL_BUG (goroutine leak / shutdown coordination)

### Recommended Action
1. Increase test timeout from 15m to 20m OR reduce TPCC fixture size
2. Investigate why import took 8m30s (baseline: check historical performance)
3. Consider splitting test into smaller, focused tests

---

**IF NO TIMEOUT DETECTED, proceed with standard analysis below:**
